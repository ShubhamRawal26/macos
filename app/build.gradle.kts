...
tasks.register("copyApks") {
    ...
    doLast {
        val buildApk = layout.buildDirectory.file("outputs/apk/debug/app-debug.apk").get().asFile
        if (buildApk.exists()) {
...
tasks.register("verifyApk") {
    doLast {
        ...
        if (!file1.exists() || file1.length() < 1_000_000) {
            throw GradleException("Verification failed: File 1 does not exist or is smaller than 1MB.")
        }
        if (!file2.exists() || file2.length() < 1_000_000) {
            throw GradleException("Verification failed: File 2 does not exist or is smaller than 1MB.")
        }
        println("Airtight verification successful! Both APK files are valid, complete, and exceed 1MB in size.")
    }
}
